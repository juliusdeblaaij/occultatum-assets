...
def rasterize_window(window_df, window_bounds, out_shape=(512, 512), out_image="window_raster.png", out_legend="window_raster_legend.png"):
    ...
    The raster output contains the soil type index (starting from 1) as used in get_soil_types.
    """
    x0, y0, x1, y1 = window_bounds
    shapes = []
    soil_types = window_df["element"].unique().to_list()
    soil_to_index = {soil: i for i, soil in enumerate(soil_types)}
    # Use local soil types for coloring
    local_soil_colors = {soil: SOIL_COLORS.get(soil, "#BEE8FF") for soil in soil_types}
    for row in window_df.iter_rows(named=True):
        soil = row["element"]
        color = local_soil_colors[soil]
        shapes.append((row["geometry"], soil_to_index[soil]))
    ...
    # Build RGB image from local soil colors
    int_to_rgb = {soil_to_index[soil]: hex_to_rgb(local_soil_colors[soil]) for soil in soil_types}
    rgb = np.zeros((out_shape[0], out_shape[1], 3), dtype=np.uint8)
    for val, color in int_to_rgb.items():
        mask = raster == val
        rgb[mask] = color
    imageio.imwrite(out_image, rgb)
    save_legend(out_legend, local_soil_colors)
    return raster, rgb
...
def get_all_soil_types():
    """
    Returns a list of all soil types as a map from soil type index to name
    """
    return {i: soil for i, soil in enumerate(SOIL_COLORS.keys())}

def get_soil_types_for_window(window_df):
    """
    ...
    soil_types = window_df["element"].unique().to_list()
    filtered_soil_types = {i: soil for i, soil in enumerate(SOIL_COLORS.keys()) if soil in soil_types}
    return filtered_soil_types

def get_soil_color_map():
    """
    Returns a dict mapping soil type index (from get_all_soil_types) to matplotlib RGB color tuple.
    """
    all_soil_types = get_all_soil_types()  # {idx: soil_name}
    return {idx: SOIL_COLORS[soil_name] for idx, soil_name in all_soil_types.items() if soil_name in SOIL_COLORS}
...
if __name__ == "__main__":
    ...
    raster, rgb = rasterize_window(window_df, window_bounds)
