...
# --- Raster caching mechanism ---
raster_csv_path = f"raster_{seed}.csv"

if os.path.exists(raster_csv_path):
    print(f"Raster for seed {seed} already exists. Loading from {raster_csv_path}")
    raster = np.loadtxt(raster_csv_path, delimiter=",", dtype="uint8")
    found = True
    nrows, ncols = raster.shape

    # Ensure raster shape matches amount_of_tiles_per_side
    if nrows != amount_of_tiles_per_side or ncols != amount_of_tiles_per_side:
        raise ValueError(f"Raster shape {raster.shape} does not match expected {amount_of_tiles_per_side}x{amount_of_tiles_per_side}")

    # --- Soil type and group mappings (must match those in the else branch) ---
    WATER_TYPES = {
        "estuary": "#73DFFF",
        "tidal flats": "#448970",
    ...
    # --- Rebuild patch attribute arrays from raster and mappings ---
    soil_type_idx_arr = raster.copy()
    soil_type_name_arr = np.empty((amount_of_tiles_per_side, amount_of_tiles_per_side), dtype='<U32')
    soil_group_idx_arr = np.zeros((amount_of_tiles_per_side, amount_of_tiles_per_side), dtype=int)
    soil_group_name_arr = np.empty((amount_of_tiles_per_side, amount_of_tiles_per_side), dtype='<U32')
    pcolor_arr = np.empty((amount_of_tiles_per_side, amount_of_tiles_per_side, 3), dtype=int)
    ...
    for i in range(amount_of_tiles_per_side):
        for j in range(amount_of_tiles_per_side):
            idx = int(raster[i, j])
            ...
            soil_type_name_arr[i, j] = soil_name
    ...
    while not found:
        ...
        if 'element' in window_gdf.columns:
            ...
            if (
                ...
                found = True
                # Save raster to CSV
                np.savetxt(raster_csv_path, raster, fmt="%d", delimiter=",")
        attempt += 1
    ...
    if nrows != amount_of_tiles_per_side or ncols != amount_of_tiles_per_side:
        raise ValueError(f"Raster shape {raster.shape} does not match expected {amount_of_tiles_per_side}x{amount_of_tiles_per_side}")
    soil_type_idx_arr = np.zeros((amount_of_tiles_per_side, amount_of_tiles_per_side), dtype=int)
    soil_type_name_arr = np.zeros((amount_of_tiles_per_side, amount_of_tiles_per_side), dtype='<U32')
    soil_group_idx_arr = np.zeros((amount_of_tiles_per_side, amount_of_tiles_per_side), dtype=int)
    soil_group_name_arr = np.zeros((amount_of_tiles_per_side, amount_of_tiles_per_side), dtype='<U32')
    pcolor_arr = np.empty((amount_of_tiles_per_side, amount_of_tiles_per_side, 3), dtype=int)
...
# Set the NetLogo global variable 'soil-type-names' to a list of soil type names
soil_type_names_list = list(SOIL_TYPE_ORDER)
# Build a NetLogo list string: ["name1" "name2" ...]
netlogo_soil_type_names = '[{}]'.format(' '.join(f'"{name}"' for name in soil_type_names_list))
netlogo.command(f'set soil-type-names {netlogo_soil_type_names}')

landscape_type_names_list = [name for name, _ in SOIL_GROUPS]
# Build a NetLogo list string: ["name1" "name2" ...]
